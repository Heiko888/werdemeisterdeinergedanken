...
// Stil
// ===========================================================================
// hell=true → Creme-Theme (Grund #f6f4ee, dunkle Tinte-Schrift). Gefüllte
// Gold-Chips (Nummern, Badge, CTA-Button) bleiben golden mit dunklem Text.
const cssFor = (F, hell) => `
...
.pageno{position:absolute;top:${F.padTop + 14}px;right:64px;font-size:22px;font-weight:700;letter-spacing:2px;color:${hell ? "#7e6410" : "rgba(242,212,137,.9)"};z-index:6}
...
.tick{width:66px;height:6px;border-radius:4px;background:${hell ? "linear-gradient(100deg,#d9a93a,#7e6410)" : "linear-gradient(100deg,#f2d489,#d9a93a)"};margin-bottom:28px}
.kicker{font-size:22px;font-weight:700;letter-spacing:3px;text-transform:uppercase;color:${hell ? "#7e6410" : "#f2d489"};margin-bottom:20px}
...
/* Cover */
.cover-eb{font-size:24px;font-weight:700;letter-spacing:3px;text-transform:uppercase;color:${hell ? "#7e6410" : "#f2d489"};margin-bottom:26px}
...
.fcheck{flex:none;width:40px;height:40px;border-radius:50%;display:flex;align-items:center;justify-content:center;
 background:${hell ? "rgba(217,169,58,.16)" : "rgba(242,212,137,.16)"};border:2px solid ${hell ? "rgba(126,100,16,.45)" : "rgba(242,212,137,.5)"}}
...
.plan.best{border-color:${hell ? "rgba(168,132,42,.6)" : "rgba(242,212,137,.55)"};background:${hell ? "rgba(232,193,95,.16)" : "rgba(242,212,137,.08)"}}
...
/* CTA */
.cta-action{margin-top:30px;padding-left:24px;border-left:6px solid;border-image:${hell ? "linear-gradient(120deg,#d9a93a,#7e6410)" : "linear-gradient(120deg,#f2d489,#d9a93a)"} 1;font-size:36px;line-height:1.35;color:${hell ? "#16231f" : "#f4f2ec"};font-weight:600}
...
const CHECK = (hell) => `<svg viewBox="0 0 24 24" fill="none" stroke="${hell ? "#7e6410" : "#f2d489"}" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><path d="M20 6 9 17l-5-5"/></svg>`;

const dots = (i, total) =>
  `<div class="dots">${Array.from({ length: total }, (_, k) => `<span class="dot ${k === i ? "on" : ""}"></span>`).join("")}</div>`;

function inner(s, hell) {
  switch (s.role) {
    case "cover":
    ...
    case "features":
      return `<div class="wrap"><div class="tick"></div><div class="kicker">${s.kicker}</div><div class="lead">${s.lead}</div>
        <div class="feat">${s.items.map(([t, d]) => `<div class="fitem"><div class="fcheck">${CHECK(hell)}</div><div class="ftext"><div class="ft">${t}</div><div class="fd">${d}</div></div></div>`).join("")}</div></div>`;
    case "steps":
      return `<div class="wrap"><div class="tick"></div><div class="kicker">${s.kicker}</div><div class="lead">${s.lead}</div>
...
function docFor(F, s, i, total, overlay = false, hell = false) {
...
<style>${cssFor(F, hell)}</style></head><body${bodyStyle}>${layers}
${brainmini}${pageno}${inner(s, hell)}${foot}</body></html>`;
}
...
for (const F of FORMATS) {
  ...
  // Marken-Hintergrund (Ebene 1 in Canva) – nur bg + Sterne, dunkel + Creme.
  ...
  for (const hell of [false, true]) {
    ...
    const tmp = join(HERE, `.wbg-${F.key}${hell ? "-hell" : ""}.html`);
    writeFileSync(tmp, `<!doctype html><html><head><meta charset="utf8"><style>${cssFor(F, hell)}</style></head><body><div class="bg"></div><div class="stars"></div></body></html>`);
    await pg.goto(pathToFileURL(tmp).href, { waitUntil: "networkidle" });
    await pg.screenshot({ path: join(overlayDir, hell ? "_hintergrund-hell.png" : "_hintergrund.png") });
    await pg.close();
    rmSync(tmp, { force: true });
  }
  for (let i = 0; i < SLIDES.length; i++) {
    // Je Folie zwei Themes: dunkel (Standard) und hell (-hell).
    for (const hell of [false, true]) {
      ...
      // 1) Fertige Folie mit Marken-Hintergrund.
      {
        ...
        writeFileSync(tmp, docFor(F, SLIDES[i], i, total, false, hell));
        await pg.goto(pathToFileURL(tmp).href, { waitUntil: "networkidle" });
      ...
      // 2) Transparentes Overlay für Canva (Foto kommt in Canva darunter).
      {
        ...
        writeFileSync(tmp, docFor(F, SLIDES[i], i, total, true, hell));
        await pg.goto(pathToFileURL(tmp).href, { waitUntil: "networkidle" });
  ...
  console.log("✓", F.key, `(${total} Folien + ${total} Overlays) ×2`);
}
await browser.close();

// Serie M „Übung & Meisterschaft" (6 Posen: Arme verschränkt seitl., Laufen, Punkt,
// Nachdenken, Gehen/Zeigen, Faust) mit Weisheiten zu Wiederholung, Dranbleiben,
// Wachstum. Zentrierter Aufbau (Text oben, Figur mittig-unten), je 4:5 + 9:16.
...
function page(fmtKey, item, bg) {
  ...
  const p = place(item.pose, f.targetH, f.w / 2, f.footY);
...
ITEMS.forEach((item, i) => {
  ...
  fs.writeFileSync(path.join(BUILD, `uebung-${nn}-${item.key}-4x5.html`), page("4x5", item, bg));
  fs.writeFileSync(path.join(BUILD, `uebung-${nn}-${item.key}-9x16.html`), page("9x16", item, bg));
});
console.log(`Serie M „Übung & Meisterschaft": ${ITEMS.length}x2 Seiten -> tools/social/weisheiten/build/`);

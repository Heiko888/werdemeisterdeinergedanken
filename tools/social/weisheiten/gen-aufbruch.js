// Serie G „Aufbruch & Energie" (6 dynamische Posen: Sprung, Laufen, Hand reichen,
// Gehen/Zeigen, offene Arme, Finger hoch) mit energetischen Weisheiten. Zentrierter
// Aufbau (Text oben, Figur mittig-unten), je Motiv 4:5 (1080x1350) + 9:16 (1080x1920).
...
// 6 Motive: Pose + Weisheit (kurze Zeilen, goldene Schlüsselwörter). lift = px anheben
// (für Sprung, damit die Figur „fliegt"); scale = Feinjustierung der Figurhöhe.
const ITEMS = [
  { n: 1, key: "sprung",  pose: "sprung",     lift: 150, scale: 0.92,
    q: `Der <em>Sprung</em><br>ist k&uuml;rzer als<br>die <em>Angst</em> davor.` },
  { n: 2, key: "weg",     pose: "rennt",      lift: 0,   scale: 1.0,
    q: `Wer <em>losl&auml;uft</em>,<br>findet den<br><em>Weg</em>.` },
  { n: 3, key: "jetzt",   pose: "reichtHand", lift: 0,   scale: 1.0,
    q: `<em>Komm mit</em>.<br>Dein Weg<br>beginnt <em>jetzt</em>.` },
  { n: 4, key: "schritt", pose: "gehtZeigt",  lift: 0,   scale: 1.0,
    q: `Der erste<br><em>Schritt</em><br>geh&ouml;rt <em>dir</em>.` },
  { n: 5, key: "offen",   pose: "offeneArme", lift: 0,   scale: 0.96,
    q: `&Ouml;ffne dich.<br>Das <em>Neue</em><br>ist schon <em>da</em>.` },
  { n: 6, key: "entscheidung", pose: "merke", lift: 0,   scale: 1.0,
...
ITEMS.forEach((item, i) => {
  ...
  fs.writeFileSync(path.join(BUILD, `aufbruch-${nn}-${item.key}-4x5.html`), page("4x5", item, bg));
  fs.writeFileSync(path.join(BUILD, `aufbruch-${nn}-${item.key}-9x16.html`), page("9x16", item, bg));
});
console.log(`Serie G „Aufbruch & Energie": ${ITEMS.length}x2 Seiten -> tools/social/weisheiten/build/`);

// Einzel-Story „Papa, kannste mal erklären?" (9:16, 1080x1920) mit ZWEI Personen:
// Junge (Blick nach oben, Rucksack) links + Papa (Hände in den Taschen, Blick nach
// unten) rechts, einander zugewandt. Marken-Look, Text oben, Logo unten (Story-safe).
...
const FOOT = 1885; // gemeinsame Fußlinie
...
function place(pose, targetH, centerX) {
  const bb = BBOX[pose];
...
function figureHTML(fig) {
  const p = place(fig.pose, fig.targetH, fig.centerX);
  return `<div class="figure" style="z-index:${fig.z};left:${p.left}px;top:${p.top}px;width:${p.dispW}px;height:${p.dispH}px;">
...
.content{position:absolute;z-index:4;top:128px;left:50%;transform:translateX(-50%);width:900px;text-align:center;}
h1{margin:0;font-size:74px;line-height:1.12;font-weight:500;letter-spacing:-.5px;color:var(--cream);text-shadow:0 2px 36px rgba(9,11,16,.78);}
...
.rule{margin:28px auto 0;width:70px;height:4px;border-radius:2px;background:linear-gradient(90deg,var(--gold-400),var(--gold-500));box-shadow:0 0 16px rgba(217,169,58,.5);}
.brand{position:absolute;bottom:120px;left:50%;transform:translateX(-50%);z-index:6;display:inline-flex;align-items:center;gap:15px;filter:drop-shadow(0 2px 20px rgba(9,11,16,.8));}
.brand img{height:62px;width:auto;display:block;filter:drop-shadow(0 0 18px rgba(217,169,58,.28));}
...
fs.writeFileSync(path.join(BUILD, `papa-papa-erklaeren-9x16.html`), page(a.backdrops[0]));
console.log(`Story „Papa, kannste mal erklären?" (2 Personen): 1 Seite -> tools/social/weisheiten/build/`);

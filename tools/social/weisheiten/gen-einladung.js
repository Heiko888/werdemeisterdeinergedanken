...
ITEMS.forEach((item, i) => {
  ...
  fs.writeFileSync(path.join(BUILD, `einladung-${nn}-${item.key}-4x5.html`), page("4x5", item, bg));
  fs.writeFileSync(path.join(BUILD, `einladung-${nn}-${item.key}-9x16.html`), page("9x16", item, bg));
});

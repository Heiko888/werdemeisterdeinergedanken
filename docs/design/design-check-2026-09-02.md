...
**2026-09-03 — Durchgang 4 (L3 Homepage-Takt, mit App + Freigabe):**

| Fund | Umsetzung | Datei |
|------|-----------|-------|
| **L3** | LeadMagnet/E-Book-Sektion (mittlere der drei hellen) auf **Dunkel** umgestellt → sauberer Wechsel Testimonials(dunkel) → MaybeNotYou(hell) → **LeadMagnet(dunkel)** → Faq(hell) → FinalCta(dunkel). | `LeadMagnet.tsx` |
| **System** | Neue wiederverwendbare **`.on-light`**-Utility (Gegenstück zu `.on-dark`): setzt die ink-/accent-Token auf Hell-Werte zurück — für eine helle Karte innerhalb einer dunklen Sektion. Die weiße Formular-Karte in LeadMagnet nutzt sie, damit ihr Text lesbar bleibt. | `globals.css` |

...
flowchart LR
  subgraph Q[Datenquellen]
    ...
    inl["Inline-Daten im Skript<br/>(Cover, Marketing-Serien)"]:::src
  end
...
| Carousel-PNG | `npm run carousels:png` | Carousel-HTMLs (baut selbst) | `docs/carousels/export/**` (PNG) |
...
| Reel-Cover-PNG | `npm run covers:png` | Cover-HTMLs (**Vorlauf nötig**) | `docs/reels/covers/export/**` (PNG) |
...
| Brand-Assets | `node docs/marketing/brand-assets.mjs` | Inline | `docs/marketing/**/*.png` |
...
`carousel-texte.mjs`, `intro-video-drehbuch.mjs`, `marketing-serien.mjs`,
`stufen-ueberblick.mjs`.
...
- **`sharp`** — **fehlt in `package.json`/`node_modules`** und ist für
  `npm run vorlagen:galerie` zwingend nachzuinstallieren.

...
/**
 * Liefert das gekaufte Buch als PDF-Anhang (Edition „pdf").
 * Wird nach erfolgreicher Zahlung vom Stripe-Webhook aufgerufen.
 */
export async function sendBuchPdfMail(resend: Resend, to: string): Promise<void> {
  ...
  const html = shell(`
    ...
    <p style="margin:0 0 1rem;font-size:16px;line-height:1.6;color:#2a3446">
      vielen Dank für deinen Kauf! Im Anhang findest du dein Exemplar von
    ...
    <p style="font-size:12px;line-height:1.5;color:#9aa4b5;margin:0">
      ...
      Sollte der Anhang fehlen oder etwas nicht stimmen, antworte einfach auf
      diese E-Mail – wir kümmern uns persönlich.
    </p>
  `);

  const text = `Dein Buch ist da.

Vielen Dank für deinen Kauf! Im Anhang findest du dein Exemplar von „Werde Meister deiner Gedanken" als PDF – zum Lesen am Computer, Tablet oder Smartphone.

Mein Tipp: Lies in deinem Tempo, Kapitel für Kapitel.

Du erhältst diese E-Mail, weil du das Buch auf ${site.url} gekauft hast. Sollte der Anhang fehlen, antworte einfach auf diese E-Mail.
